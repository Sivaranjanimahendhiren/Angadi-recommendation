...
def get_sentiment_scores():
    ...
    avg_sentiment = {pid: sum(scores)/len(scores) for pid, scores in product_sentiments.items() if scores}
...
def combine_scores(interaction_scores, sentiment_scores):
    ...
    for pid in interaction_scores:
        combined[pid] += interaction_scores.get(pid, 0)
        if pid in sentiment_scores:
            combined[pid] += sentiment_scores[pid] * 2.0  # Give higher weight to sentiment
...
def lambda_handler(event=None, context=None):
    try:
        ...
        # Apply low threshold to increase likelihood of results
        MIN_SCORE = 0.5

        sorted_scores = sorted(final_scores.items(), key=lambda x: x[1], reverse=True)
        top_products = [item for item in sorted_scores if item[1] >= MIN_SCORE][:10]
